...
class ReaderButtonSwitchCam(QThread):
    ...
    def _handle_can_message(self, msg):
        ...
        # Cập nhật time và command
        self.last_time = current_time

        # Nếu là lệnh mới → reset state
        if self.last_command != last_two_bytes:
            self.command_consumed = False
            
        self.last_command = last_two_bytes
        
        # Nếu lệnh đã dùng rồi & không phải hold liên tục (zoom) → bỏ qua
        if self.command_consumed and action not in ("zoom_in", "zoom_out"):
            return
