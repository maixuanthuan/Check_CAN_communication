...
class MainWindow(QMainWindow):
    ...
    def _on_switch_camera_clicked(self):
        ...
        self.video_widget.switch_camera(self.day_mode)
        self._update_colors()
    ...
    def _handle_camera_switch(self, is_day_mode):
        ...
        if old_mode != self.day_mode:
            ...
            mode_text = "NGÀY" if self.day_mode else "ĐÊM"
    ...
    def _auto_check_day_night(self):
        """Kiểm tra thời gian mỗi phút nếu đang ở chế độ Auto."""
        ...
        new_day_mode = 6 <= current_hour < 18

        if new_day_mode != self.day_mode:
            self.day_mode = new_day_mode
            self.video_widget.switch_camera(self.day_mode)
            self._update_colors()
            print(f"[AUTO] Thời gian thay đổi → chuyển sang {'NGÀY' if self.day_mode else 'ĐÊM'}.")
